...
import { buscarSugestoesPacientes } from '@/services/agenda.service'
import AgendaToolbar from '@/components/agenda/AgendaToolbar'
import AgendaCalendar from '@/components/agenda/AgendaCalendar'

export default function AgendaPacientesPage() {
  ...
  useEffect(() => {
    setHeader('Central de Agenda — Pacientes', 'Visualize e gerencie os agendamentos por paciente')
  }, [setHeader])

  const agenda = useAgenda('pacientes', { autoLoad: false })
  ...
  useEffect(() => {
    ...
    buscarSugestoesPacientes(inputValue.trim()).then(setSuggestions)
    ...
    <div className="bg-white border border-slate-100 rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.05)] flex flex-col items-center justify-center min-h-105 gap-4">
      ...
      <div className="text-center space-y-1">
        ...
        <p className="text-[13px] text-slate-400 font-normal leading-relaxed">
          Pesquise um paciente para visualizar os atendimentos
        </p>
      </div>
    </div>
  )
