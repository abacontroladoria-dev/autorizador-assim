    ...
    <div style={{ background: "white", borderRadius: "14px", border: "1px solid #e5e7eb", boxShadow: "0 1px 4px rgba(0,0,0,.06)", padding: "16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px", paddingBottom: "12px", borderBottom: "1px solid #f0f0f0", flexWrap: "wrap", gap: "8px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <span style={{ fontSize: "12px", color: "#9ca3af" }}>{rec.length} registros · 💾</span>
          {rec.length > 0 && (
            ...
            <tbody>
              {rec.map((r, i) => (
                <tr key={i} style={{ borderTop: "1px solid #f0f0f0" }}>
                  ...
                  <td style={{ padding: "8px 12px", color: "#6b7280", fontSize: "12px" }}>{fmtName(r.profissional)}</td>
                  <td style={{ padding: "8px 12px" }}>
                    <span style={{ background: B.blueLt, color: B.blue, borderRadius: "999px", padding: "2px 8px", fontSize: "11px" }}>{r.especialidade}</span>
                  </td>
                  <td style={{ padding: "8px 12px", color: "#6b7280", fontSize: "12px" }}>{r.unidade}</td>
                  <td style={{ padding: "8px 12px", color: "#6b7280", fontSize: "12px" }}>{r.dia}</td>
                  <td style={{ padding: "8px 12px", fontWeight: 700, fontSize: "12px" }}>{r.hora}</td>
                  <td style={{ padding: "8px 12px", color: "#9ca3af", fontSize: "11px" }}>{r.registradoEm}</td>
                  <td style={{ padding: "8px 12px" }}>
