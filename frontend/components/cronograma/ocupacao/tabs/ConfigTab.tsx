      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "16px" }}>
          {REGRAS_LEGENDA.map(({ r, c, title, desc }) => (
            <div key={r} style={{ display: "flex", gap: "10px", padding: "10px", background: "var(--card)", borderRadius: "8px", border: "1px solid var(--border)", fontSize: "12px" }}>
              ...
              <div><strong style={{ color: B.navy }}>{title}</strong> — <span style={{ color: "var(--muted-foreground)", lineHeight: "1.5" }}>{desc}</span></div>
            </div>
          ))}
        </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: "12px", marginTop: "4px" }}>
          ...
          <input
            ...
            type="password"
            style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "8px 12px", fontSize: "13px", width: "100%", fontFamily: "monospace", boxSizing: "border-box" }}
          />
        </div>
      </div>
