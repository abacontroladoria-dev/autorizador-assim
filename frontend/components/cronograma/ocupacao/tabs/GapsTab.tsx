    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
      <div style={{ background: "white", borderRadius: "14px", border: "1px solid #e5e7eb", boxShadow: "0 1px 4px rgba(0,0,0,.06)", padding: "14px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center" }}>
          ...
          <select value={gapEsp} onChange={e => setGapEsp(e.target.value)} style={{ ...inputSt, background: "white" }}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", cursor: "pointer", color: gapTudoZero ? "#dc2626" : "#6b7280", fontWeight: gapTudoZero ? 700 : 400 }}>
            <input type="checkbox" checked={gapTudoZero} onChange={e => setGapTudoZero(e.target.checked)} style={{ accentColor: "#dc2626" }} />
            Sem nada agendado
          </label>
          {gapSearch && <button onClick={() => setGapSearch("")} style={{ fontSize: "12px", color: "#9ca3af", background: "none", border: "none", cursor: "pointer" }}>✕</button>}
        </div>
      </div>
      ...
      {!rows.length ? (
        <div style={{ background: "white", borderRadius: "14px", border: "2px dashed #e5e7eb", padding: "32px 24px", textAlign: "center" }}>
          ...
          <div style={{ color: "#9ca3af", fontSize: "14px" }}>Nenhum gap com esses filtros</div>
        </div>
      ) : (
              ...
              <thead style={{ background: "#f8fafc", position: "sticky", top: 0 }}>
                <tr>
                  {["Paciente", "Especialidade", "Autorizado", "Ofertado", "Diferença"].map(h => (
                    <th key={h} style={{ textAlign: h === "Paciente" || h === "Especialidade" ? "left" : "center", padding: "8px 12px", fontSize: "11px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".04em" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
