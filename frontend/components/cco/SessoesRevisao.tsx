...
export default function SessoesRevisao({ sessoes, loading }: Props) {
    ...
    <div className="bg-card border border-border rounded-xl p-6 flex flex-col">
      <div className="mb-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">Sessões em Revisão</h3>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-1 overflow-y-auto max-h-96 -mr-2 pr-2">
          {lista.map(s => (
            <div
              key={s.id}
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-foreground/[0.03] transition-colors"
            >
