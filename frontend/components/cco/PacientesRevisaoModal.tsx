...
export default function PacientesRevisaoModal({ open, onClose, sessoes, onPacienteClick }: Props) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-card border border-border rounded-xl shadow-lg max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-4 shrink-0">
            <div>
              <h2 className="text-base font-semibold text-foreground">Pacientes em Revisão</h2>
                ...
                return (
                  <div key={paciente.nome}>
                    <button
                      onClick={(e) => {
                        if (e.detail === 1) {
                          setExpandedPaciente(isExpanded ? null : paciente.nome)
                        } else if (e.detail === 2) {
                          onPacienteClick?.(paciente.nome)
                        }
                      }}
                      className="w-full flex items-center gap-3 px-6 py-4 hover:bg-foreground/2 transition-colors text-left"
                      title="Clique duas vezes para detalhes"
                    >
                          ...
                          <div key={sessao.id} className="flex items-center justify-between text-xs">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-foreground/50 text-[11px] mt-0.5">
                                {sessao.terapeutaOriginal} → {sessao.terapeutaSubstituto}
                              </p>
                            </div>
