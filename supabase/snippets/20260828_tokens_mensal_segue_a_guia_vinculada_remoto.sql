-- =============================================================================
-- get_tokens_mensal passa a seguir a guia VINCULADA (caso Kourtney)
-- =============================================================================
-- Aplica 20260828180000_tokens_mensal_segue_a_guia_vinculada.sql.
--
-- Contexto: 20260828170000 (já aplicada) fez get_tokens_mensal excluir bloco
-- reclassificado (caso Benjamim Vilazio — confirmado funcionando). Mas o caso
-- da Kourtney Savino Lope (03/08, vínculo, não reclassificação) continuava
-- ausente da Conferência de Filipetas mesmo com get_auditoria_assim já
-- mostrando forma_autorizacao = 'Erro no Reconhecimento Facial' para o bloco.
-- Esta migration ensina get_tokens_mensal a seguir a guia vinculada, não só a
-- fila — o mesmo padrão que 20260827000004 já aplicou em
-- get_auditoria_assim_periodo.
--
-- CORRIGE UMA TENTATIVA ANTERIOR DESTA MESMA VERSÃO (ainda não registrada no
-- livro-caixa): o primeiro CREATE OR REPLACE trazia guia/token do vínculo mas
-- deixava forma_autorizacao com fo.forma_autorizacao NA FRENTE do vínculo no
-- COALESCE — o mesmo erro que 20260827000003 cometeu e a 20260827000004
-- corrigiu na RPC diária. Medido: get_tokens_mensal já trazia guia 15032, mas
-- forma_autorizacao continuava 'QR Code' (era para ser 'Erro no Reconhecimento
-- Facial'). Corrigido invertendo a ordem do COALESCE. Como a versão anterior
-- nunca chegou a ser registrada no livro-caixa, não há necessidade de nova
-- migration — este arquivo já reflete a definição corrigida.
--
-- Tudo ou nada: erro no bloco desfaz o pacote inteiro.
-- =============================================================================
...
VALUES
  ('20260828180000', 'tokens_mensal_segue_a_guia_vinculada')
ON CONFLICT (version) DO NOTHING;

commit;

-- Confirmação: tem de aparecer.
SELECT version, name
FROM supabase_migrations.schema_migrations
WHERE version = '20260828180000';

-- ─────────────────────────────────────────────────────────────────────────
-- VERIFICAÇÃO DO CASO REAL (rodar depois do commit)
-- ─────────────────────────────────────────────────────────────────────────

-- Esperado agora: guia 15032, forma_autorizacao = 'Erro no Reconhecimento
-- Facial' (não mais 'QR Code').
SELECT bloco_id, guia, token, forma_autorizacao
  FROM get_tokens_mensal('2026-08-01')
 WHERE bloco_id = '11649_2026-08-03_22070435_11:20:00';

-- Confere que o Benjamim continua fora (sem regressão da 20260828170000).
SELECT bloco_id, guia, token, forma_autorizacao
  FROM get_tokens_mensal('2026-08-01')
 WHERE paciente_nome ILIKE '%Benjamim Vilazio%'
