-- =============================================================================
-- Resumo diário da Auditoria ASSIM — aplicação remota + backfill
-- =============================================================================
-- Empacota a migration `20260824050000_resumo_diario_auditoria_assim.sql` para
-- o SQL Editor. Ver supabase/snippets/README.md: `db push` não seleciona
-- arquivo, empurra o pendente inteiro.
--
-- ORDEM DE EXECUÇÃO — rode UM BLOCO POR VEZ, nesta ordem. Não cole o arquivo
-- inteiro de uma só vez: o bloco 3 é demorado e, rodando junto com o resto,
-- seguraria uma das 10 conexões do pool por minutos — que é exatamente como o
-- sistema inteiro virou 504 em 24/08.
--
--   1. A migration inteira (tabela + 2 funções + cron + livro-caixa). Este
--      bloco pode e deve ser colado de uma vez só.  ← É O ÚNICO OBRIGATÓRIO.
--   2. Conferência: as duas funções existem, o cron está agendado, a tabela
--      não ficou legível pelo anon.
--   3. Backfill do histórico, UM MÊS POR VEZ. Opcional — sem ele a tela já
--      funciona, só começa mostrando os últimos 7 dias.
--   4. Paridade contra a tela diária. É a verificação que importa.
--
-- Depois do bloco 1 a tela já responde. Antes dele, o modal só mostra estado de
-- erro, porque `get_auditoria_assim_resumo` não existe.
-- =============================================================================


-- ─────────────────────────────────────────────────────────────────────────────
-- BLOCO 1 — a migration (cole DESTE ponto até o fim do bloco)
-- ─────────────────────────────────────────────────────────────────────────────
-- Idempotente: testado aplicando duas vezes seguidas no Postgres local, exit 0
-- nas duas. Pode rodar inteiro de uma vez.
...
-- Registrar no livro-caixa, senão o CLI vai querer reaplicar depois:
insert into supabase_migrations.schema_migrations (version, name)
values ('20260824050000', 'resumo_diario_auditoria_assim')
on conflict (version) do nothing;


-- ─────────────────────────────────────────────────────────────────────────────
-- BLOCO 2 — conferência do que entrou
-- ─────────────────────────────────────────────────────────────────────────────
select p.proname, pg_get_function_identity_arguments(p.oid) as args, p.prosecdef as security_definer
from pg_proc p
join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in ('refresh_auditoria_assim_resumo', 'get_auditoria_assim_resumo')
order by 1;

select jobname, schedule, active
from cron.job
where jobname like 'refresh-auditoria-assim-resumo%'
order by 1;

-- A tabela não pode estar acessível ao anon: a leitura passa pela RPC.
select grantee, privilege_type
from information_schema.role_table_grants
where table_name = 'auditoria_assim_resumo_diario'
order by 1, 2;


-- ─────────────────────────────────────────────────────────────────────────────
-- BLOCO 3 — backfill do histórico, UM MÊS POR VEZ
-- ─────────────────────────────────────────────────────────────────────────────
-- Por que em lotes e não `refresh(...'2026-01-01', current_date)` de uma vez:
-- a função itera dia a dia chamando `get_auditoria_assim`, e cada dia custa
-- segundos. Um semestre inteiro numa transação seguraria uma das 10 conexões
-- do pool por muitos minutos — que é exatamente como o sistema inteiro virou
-- 504 em 24/08.
--
-- `p_forcar => true` é necessário aqui: os dias antigos nascem `fechado` e o
-- caminho normal os pula (é o que impede o cron de crescer sem limite).
--
-- Rode UMA linha por vez, conferindo o retorno antes de seguir. O número
-- devolvido é quantas linhas de resumo o mês gerou.

select public.refresh_auditoria_assim_resumo('2026-07-01', '2026-07-31', true) as linhas_julho;
-- select public.refresh_auditoria_assim_resumo('2026-06-01', '2026-06-30', true) as linhas_junho;
-- select public.refresh_auditoria_assim_resumo('2026-05-01', '2026-05-31', true) as linhas_maio;
-- select public.refresh_auditoria_assim_resumo('2026-04-01', '2026-04-30', true) as linhas_abril;

-- Até onde vale a pena voltar: a tabela `agenda_tita` não tem nada antes de
-- 01/07 em volume confiável (ver o congelamento da grade), então meses
-- anteriores tendem a devolver pouco ou nada. Confira antes de insistir:
select date_trunc('month', data_atendimento)::date as mes, count(*) as sessoes_assim
from public.agenda_tita
where ativo = true and convenio_nome ilike '%assim%'
group by 1 order by 1;


-- ─────────────────────────────────────────────────────────────────────────────
-- BLOCO 4 — paridade: o resumo tem de bater com a tela diária
-- ─────────────────────────────────────────────────────────────────────────────
-- Esta é a verificação que importa. Troque a data por um dia com movimento
-- variado (de preferência um que tenha glosa, falta e token) e confira que
-- todas as linhas dizem OK.
--
-- Ela compara, por situação, o que as RPCs da tela devolvem contra o que o
-- resumo guardou. Divergência aqui significa que o modal e o card mostrariam
-- números diferentes para o mesmo dia — que é o defeito que este desenho
-- inteiro existe para impedir.
with esperado as (
  select situacao, count(*)::int as n
  from public.get_auditoria_assim('2026-08-20'::date)
  group by 1
  union all
  select case when tipo_falta ilike '%terapeuta%' then 'FALTA_TERAPEUTA' else 'FALTA' end, count(*)::int
  from public.get_faltas_auditoria_assim('2026-08-20'::date)
  group by 1
),
obtido as (
  select situacao, sum(sessoes)::int as n
  from public.auditoria_assim_resumo_diario
  where data = '2026-08-20'::date
  group by 1
)
select
...
-- Quantos dias já têm resumo, e até quando o cron chegou.
select
