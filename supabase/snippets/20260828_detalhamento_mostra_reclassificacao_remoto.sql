-- =============================================================================
-- Empacotamento para o SQL Editor — 20260828190000_detalhamento_mostra_reclassificacao.sql
-- =============================================================================
-- Ver supabase/snippets/README.md para o porquê deste caminho de deploy.
--
-- O QUE ISTO CORRIGE
-- Benjamim Vilazio Kmiciak, glosa de 26/08/2026 reclassificada para FALTA: o
-- detalhamento (auditoria-assim/?tab=auditoria) mostrava só "1013-CADASTRO DO
-- BENEFICI — 1013: CADASTRO DO BENEFICIARIO COM PROBLEMAS", sem nenhum sinal de
-- que a sessão foi reclassificada — sem justificativa, sem quem decidiu, sem
-- quando. A seção "Motivo da glosa" do modal só aparece quando `ehGlosa(situacao)`,
-- e reclassificar GLOSA -> FALTA muda exatamente essa situação, derrubando a
-- única seção que tinha onde mostrar isso.
--
-- Esta migration expõe 4 colunas cruas novas em get_auditoria_assim_periodo (e
-- no wrapper get_auditoria_assim): reclassificacao_situacao_anterior,
-- reclassificacao_justificativa, reclassificacao_por, reclassificacao_em. O
-- frontend (já no branch) usa isso para montar uma seção "Reclassificação
-- manual" própria no modal, visível em qualquer situação de destino.
--
-- `RETURNS TABLE` muda (4 colunas a mais), por isso o DROP + CREATE.
-- =============================================================================

begin;

-- ---------------------------------------------------------------------------
-- 20260828190000_detalhamento_mostra_reclassificacao.sql
-- ---------------------------------------------------------------------------
...
VALUES ('20260828190000', 'detalhamento_mostra_reclassificacao')
...
-- VERIFICAÇÃO
-- =============================================================================
-- 1. O caso real (Benjamim Vilazio Kmiciak, glosa reclassificada como falta em
--    26/08/2026):
--      SELECT bloco_id, situacao, situacao_anterior, reclassificado_por,
--             justificativa
--      -- (usar os nomes NOVOS na RPC: reclassificacao_situacao_anterior,
--      --  reclassificacao_justificativa, reclassificacao_por, reclassificacao_em)
--      FROM get_auditoria_assim('2026-08-26')
--      WHERE paciente_nome ILIKE '%Benjamim Vilazio%';
--    Esperado: situacao = 'FALTA', reclassificacao_situacao_anterior = 'GLOSA',
--    reclassificacao_por e reclassificacao_justificativa preenchidos.
--
-- 2. Bloco sem reclassificação continua com as 4 colunas novas NULL e nada mais
--    muda:
--      SELECT count(*) FROM get_auditoria_assim_periodo('2026-08-01','2026-08-26')
--      WHERE reclassificacao_por IS NOT NULL;
--    Compare com a contagem de reclassificações ativas no período:
--      SELECT count(*) FROM auditoria_situacao_overrides
--      WHERE desfeito_em IS NULL
--        AND bloco_id IN (SELECT bloco_id FROM get_auditoria_assim_periodo('2026-08-01','2026-08-26'));
--    Os dois números têm de bater.
-- =============================================================================
