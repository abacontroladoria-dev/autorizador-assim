-- =============================================================================
-- FASE 2 — as 5 migrations na ordem + livro-caixa. Cole INTEIRO no SQL Editor.
-- O editor roda tudo numa transacao: se uma falhar, nada e aplicado.
-- =============================================================================


-- #############################################################################
-- 20260813130000_robo_conclui_glosa.sql
-- #############################################################################
...
-- #############################################################################
-- 20260813130100_solicitar_reconhece_glosa.sql
-- #############################################################################
...
-- #############################################################################
-- 20260813130200_alerta_glosa_no_aceite.sql
-- #############################################################################
-- ============================================================================
...
-- #############################################################################
-- 20260814120000_sync_assim_conclui_pendente.sql
-- #############################################################################
...
-- #############################################################################
-- 20260814130000_central_autorizacoes_criado_por.sql
-- #############################################################################
-- Recriação FIEL de listar_central_autorizacoes (cópia byte a byte de
-- 20260813130100_solicitar_reconhece_glosa.sql) com TRÊS linhas de diferença, todas
-- para levar `criado_por` até a tela.
--
-- POR QUE
-- O card em /solicitar mostra "Processando" sem dizer de quem é a solicitação. Numa
-- recepção com várias estações isso vira pergunta em voz alta: quem pediu essa? O
-- dado já existe — fila_autorizacoes.criado_por é preenchido pelo trigger que resolve
-- machine_id -> maquinas.user_id -> usuarios.nome (20260730000000) — só não estava
-- sendo devolvido por esta função. Mesmo tratamento que cancelado_por_nome já recebe.
--
--   (1) criado_por entra na CTE ultima_fila, que é a que escolhe QUAL linha da fila
--       representa a sessão (DISTINCT ON ... ORDER BY created_at DESC). Vindo daí, o
--       nome é sempre o da mesma linha que produziu o status_final — não há como
--       mostrar o status de uma linha e o autor de outra.
--   (2) uf.criado_por no SELECT externo.
--   (3) criado_por text no fim do RETURNS TABLE.
--
-- DROP antes do CREATE porque RETURNS TABLE faz parte da assinatura: CREATE OR REPLACE
-- recusa mudança de tipo de retorno com "cannot change return type of existing function".
--
-- ATENÇÃO À ORDEM: esta migration parte da versão COM o reconhecimento de glosa
-- (20260813130100). Se ela ainda não estiver em produção, aplicar esta aqui traz a
-- mudança de glosa junto — as duas linhas do CASE que dão status_final='glosa' e
-- mostrar_na_tela=false. É o mesmo lote pendente, mas não é surpresa: está dito aqui.
--
-- A view legada vw_central_autorizacoes NÃO ganha a coluna. Ela só é usada no
-- fallback de erroColunaPacienteComplementar (solicitar/page.tsx), e ali o card
-- simplesmente não mostra o autor — degrada, não quebra.
...
-- #############################################################################
-- LIVRO-CAIXA — sem isto, o proximo `db push` reaplica tudo
-- #############################################################################
...
FROM pg_proc
WHERE proname IN ('robo_concluir_tarefa','sync_assim_results','listar_central_autorizacoes')
ORDER BY proname;
