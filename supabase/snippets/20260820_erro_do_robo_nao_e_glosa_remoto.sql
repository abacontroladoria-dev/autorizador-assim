...
-- Identico a supabase/migrations/20260820170000_erro_do_robo_nao_e_glosa.sql
--
-- O QUE RESOLVE: a auditoria mostrava "Glosa" para sessao cujo motivo era
-- "A janela da ASSIM foi fechada durante a identificacao do beneficiario" —
-- quebra do processo, nao recusa do convenio. O CASE tratava
-- `fo.status IN ('erro','glosa')` como um ramo so, e os dois estados dizem o
-- oposto: 'glosa' e a ASSIM tendo processado e recusado; 'erro' e o RPA
-- abortando antes de existir qualquer resposta.
--
-- 'erro' e 'cancelado' passam a ser a situacao 'SOLICITACAO_CANCELADA',
-- prioridade 1, contada no card "Nao Solicitadas". 'cancelado' entra junto
-- porque hoje ele cai no ramo generico e aparece como RETORNO_NAO_CONFIRMADO
-- ("enviamos e a ASSIM nao respondeu"), quando nada foi enviado.
--
-- NAO perde recusa: se o relatorio (autorizacoes_assim) tiver status diferente
-- de 'Liberado' para a sessao, os ramos de `mt` vem antes e a situacao segue
-- GLOSA. Reclassificar 'erro' so deixa de inventar as recusas que nao existem.
--
-- CUMULATIVO com 20260820150000/160000: este bloco traz a definicao FINAL de
-- get_auditoria_assim_periodo. So o CREATE OR REPLACE da funcao — nao cria
-- tabela, trigger nem grant.
--
-- A tela depende do deploy do frontend junto: o rotulo novo
-- ('Solicitacao Cancelada') e o agrupamento no card vivem lah. Aplicar so o SQL
-- deixa a linha com badge de fallback cinza e fora de todos os KPIs.
...
-- =============================================================================
-- Conferencia: como as linhas da fila do dia se distribuem depois da mudanca.
-- Esperado: nenhuma linha com situacao='GLOSA' cuja unica origem seja
-- fila_autorizacoes.status='erro'.
-- =============================================================================
select situacao, prioridade, count(*), min(observacao) as exemplo_observacao
  from public.get_auditoria_assim(current_date)
 group by situacao, prioridade
 order by prioridade, situacao;

-- =============================================================================
-- Livro-caixa
-- =============================================================================
insert into supabase_migrations.schema_migrations (version, name) values
  ('20260820170000', 'erro_do_robo_nao_e_glosa')
