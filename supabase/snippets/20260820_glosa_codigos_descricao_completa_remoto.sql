...
-- Identico a supabase/migrations/20260820150000_glosa_codigos_descricao_completa.sql
--
-- O QUE RESOLVE: o motivo da glosa aparece cortado em 25 caracteres
-- ("1403-NAO EXISTE INFORMACA"). Nao e corte nosso — a ASSIM trunca a coluna do
-- relatorio na origem, e autorizacoes_assim.codigo_erro/.descricao_erro estao
-- NULAS em 100% das linhas. O texto completo so existe no recibo do envio.
--
-- Cria o de-para public.glosa_codigos, um trigger que o alimenta a partir do
-- recibo, e faz a auditoria resolver a descricao completa. Tambem passa a
-- extrair o codigo do relatorio, que hoje fica embutido no `status` e nunca
-- chega a codigo_erro.
--
-- CUMULATIVO com 20260820140000: este bloco ja traz a definicao FINAL da
-- funcao. Rodando este, nao e preciso rodar aquele — mas as duas versoes sao
-- registradas no livro-caixa no fim, para que um `db push` futuro nao reaplique
-- a 140000 por cima e reverta esta.
--
-- Testado no Postgres local: as duas migrations aplicadas, a 150000 duas vezes
-- (idempotente), e seis cenarios conferidos (relatorio truncado com e sem
-- de-para, sufixo " *", recibo completo, recibo curto que nao rebaixa o
-- de-para, recusa sem codigo).
--
-- DEPOIS DE RODAR: completar os cinco codigos que faltam (bloco 5, no fim).

-- =============================================================================
-- 20260820150000_glosa_codigos_descricao_completa.sql
-- =============================================================================
...
-- =============================================================================
-- Conferencia 1: o vocabulario conhecido hoje. Esperado: 1013 preenchido.
-- =============================================================================
SELECT codigo, descricao, origem, atualizado_em FROM public.glosa_codigos ORDER BY codigo;

-- =============================================================================
-- Conferencia 2: como cada glosa historica passa a ser lida. `descricao_erro`
-- com 25 caracteres exatos = codigo ainda sem de-para (esperado para os cinco
-- que faltam); mais que isso = texto completo resolvido.
-- =============================================================================
SELECT status_assim,
       codigo_erro,
       descricao_erro,
       length(descricao_erro) AS tam,
       count(*)               AS sessoes
  FROM public.get_auditoria_assim_periodo(CURRENT_DATE - 120, CURRENT_DATE)
 WHERE situacao = 'GLOSA'
 GROUP BY 1,2,3,4
 ORDER BY sessoes DESC;

-- =============================================================================
-- Conferencia 3: o trigger esta no lugar e nao duplicou.
-- =============================================================================
...
INSERT INTO supabase_migrations.schema_migrations (version, name) VALUES
  ...
  ('20260820150000', 'glosa_codigos_descricao_completa')
