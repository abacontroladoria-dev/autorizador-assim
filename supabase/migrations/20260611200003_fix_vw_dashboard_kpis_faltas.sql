...
faltas AS (
  SELECT
    COUNT(DISTINCT fa.paciente_nome) FILTER (WHERE a.sala_nome ILIKE '%Realengo%')     AS realengo,
    COUNT(DISTINCT fa.paciente_nome) FILTER (WHERE a.sala_nome ILIKE '%Fazendinha%')   AS fazendinha,
    COUNT(DISTINCT fa.paciente_nome) FILTER (WHERE a.sala_nome ILIKE '%Padre Miguel%') AS "padreMiguel",
...
terapeutas AS (
  SELECT
    COUNT(DISTINCT nome_profissional) FILTER (WHERE sala ILIKE '%Realengo%')     AS realengo,
    COUNT(DISTINCT nome_profissional) FILTER (WHERE sala ILIKE '%Fazendinha%')   AS fazendinha,
    COUNT(DISTINCT nome_profissional) FILTER (WHERE sala ILIKE '%Padre Miguel%') AS "padreMiguel",
    COUNT(DISTINCT nome_profissional)                                            AS total
  FROM grade_profissionais_tita
  WHERE data = (
    SELECT COALESCE(MIN(data), CURRENT_DATE)
    FROM grade_profissionais_tita
