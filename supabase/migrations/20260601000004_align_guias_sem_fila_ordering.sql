-- Alinha a ordenação de guias na CTE guias_sem_fila de listar_central_pacientes
-- para usar data_execucao ASC (igual a vw_match_autorizacoes_assim), em vez de guia ASC.
-- Também adiciona filtro ativo=true na CTE agenda_com_tuss.
--
-- Problema 1 (ordering): os dois mecanismos de matching (Parte 2 e vw_match_autorizacoes_assim)
-- usavam critérios diferentes para numerar as guias, o que causava divergência quando
-- o número da guia não segue ordem cronológica — a Parte 2 mostrava uma guia errada
-- no card do central-pacientes (cosmético), mas a proteção contra dupla autorização
-- na página solicitar funcionava corretamente via vw_match.
--
-- Problema 2 (ativo): agenda_com_tuss lia agenda_tita sem filtrar ativo=true, permitindo
-- que sessões excluídas (ativo=false) com hora_inicial menor roubassem a guia de
-- sessões ativas substitutas com hora_inicial maior e mesmo TUSS — as sessões ativas
-- ficavam invisíveis no Controle de Pacientes por não ter guia emparelhada.
--
-- Correção: ambos os mecanismos agora usam data_execucao ASC; agenda_com_tuss filtra
-- apenas sessões ativas, alinhando o comportamento com vw_match_autorizacoes_assim.
