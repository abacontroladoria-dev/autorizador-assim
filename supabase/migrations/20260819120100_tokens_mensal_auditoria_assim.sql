-- ─────────────────────────────────────────────────────────────────────────────
-- "Token Mensal" em /auditoria-assim?tab=auditoria (pedido pós-reunião,
-- 2026-08-19): a operadora precisa ver TODOS os tokens (filipetas) do mês,
-- não só do dia, para conferir quais ainda faltam. "Fora do padrão" = não tem
-- o check de conferido — não é validação de formato do token.
--
-- get_auditoria_assim(date) hoje só enxerga 1 dia. Em vez de duplicar as ~250
-- linhas de blocos_auditoria/match_temporal numa segunda função (foi exatamente
-- essa duplicação que causou o bug de join corrigido em 20260819110000),
-- extraio a lógica para get_auditoria_assim_periodo(data_inicio, data_fim) e
-- get_auditoria_assim(date) vira um wrapper fino chamando o período com a
-- mesma data nos dois extremos. get_tokens_mensal(mes) reusa o período inteiro
-- do mês, filtrando só teve_token = true.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- get_auditoria_assim(date) vira wrapper fino — mesma assinatura e mesmas
-- colunas de retorno de antes, então CREATE OR REPLACE não precisa de DROP.
...
-- get_tokens_mensal: todos os blocos com token no mês de p_mes (qualquer dia
-- dele), pra tela "Token Mensal". Conferência (auditoria_token_conferencias)
-- é buscada pelo frontend do mesmo jeito que já faz na tabela diária —
-- mantém uma única fonte de leitura pro estado de conferido.
...
AS $function$
  SELECT
    p.bloco_id, p.paciente_id, p.paciente_nome, p.data_atendimento, p.hora_inicial,
    p.codigo_tuss, p.terapias, p.profissionais, p.guia, p.token, p.data_execucao,
    p.criado_por, p.forma_autorizacao
  FROM public.get_auditoria_assim_periodo(
    date_trunc('month', p_mes)::date,
    (date_trunc('month', p_mes) + interval '1 month' - interval '1 day')::date
  ) p
  WHERE p.teve_token = true
  ORDER BY p.data_atendimento, p.hora_inicial
$function$
;
