-- /solicitar: "Última autorização: -" ficava sumindo assim que a reconciliação
-- assíncrona vinculava a guia (ver debug do item anterior, 20260804000000).
--
-- ultima_autorizacao_anterior dependia inteiramente de match_assim — a mesma
-- heurística posicional (matrícula+dep+TUSS+ordem) que causou o bug do Samuel.
-- Além de frágil, match_assim só é alimentado pela reconciliação assíncrona
-- (`autorizacoes_assim`), então o valor demorava a aparecer e sumia assim que
-- a guia era vinculada a uma fila (NOT EXISTS de ma_auths passava a excluí-la).
--
-- Agora o RPA (`robo-autorizador/rpa.js`) captura a data/hora real da tela de
-- confirmação da ASSIM e grava em fila_autorizacoes.horario_autorizacao no mesmo
-- update que já grava numero_autorizacao. Com isso dá pra calcular
-- ultima_autorizacao_anterior direto em fila_autorizacoes, pelo paciente_id de
-- verdade — sem depender de matrícula/dep (que colide entre pacientes sem
-- carteirinha real) nem esperar a reconciliação assíncrona.
--
-- match_assim/ma_auths continuam do jeito que estão: ainda são necessários para
-- status_final='autorizado_externo'/mostrar_na_tela, que precisa detectar guias
-- autorizadas FORA do fluxo do robô (direto no portal ASSIM, sem numero_autorizacao).
--
-- Recriação FIEL de 20260804000000_fix_ma_consumos_falta_tuss.sql + a única
-- mudança: a subquery de ultima_autorizacao_anterior passa a ler de
-- fila_autorizacoes em vez de match_assim.
