-- ─────────────────────────────────────────────────────────────────────────────
-- Quebra no processo de autorização não é glosa.
--
-- O CASO QUE EXPÔS: a auditoria mostrava "Glosa" com o motivo
-- "A janela da ASSIM foi fechada durante a identificação do beneficiário."
-- Isso não é recusa do convênio: é o robô perdendo a aba no meio do caminho
-- (rpa.js:204 e :258, quando `contar()` devolve -1 porque a página morreu).
-- Ninguém do outro lado decidiu nada — a operadora começou o atendimento e a
-- janela caiu.
--
-- POR QUE ACONTECIA: até aqui o CASE tratava `fo.status IN ('erro','glosa')`
-- como um só ramo. Mas os dois estados dizem coisas opostas:
--
--   'glosa'     → a ASSIM processou e RECUSOU. O robô leu "BENEFICIO REJEITADO"
--                 no recibo do aceite; existe guia, horário e motivo. É glosa de
--                 verdade, exige tratativa, não reenvio.
--   'erro'      → o RPA abortou ANTES de qualquer desfecho (rpa.js:879, no
--                 `catch`, e só quando `concluiu` é falso). Nenhuma decisão da
--                 ASSIM foi lida. Aba fechada, dado do associado incompleto,
--                 CRM ausente, a recepção não clicou em enviar, o portal barrou
--                 o envio por beneficiário não confirmado.
--   'cancelado' → uma pessoa cancelou a solicitação antes de ela concluir.
--
-- Os dois últimos têm o mesmo desfecho operacional: a sessão NÃO tem
-- autorização e precisa ser solicitada de novo. É exatamente a régua de
-- "Não Solicitada", e não a de glosa. Chamá-los de glosa custava duas coisas:
-- inflava o número que a clínica usa para contestar recusa (uma glosa fantasma
-- nunca tem o que contestar) e escondia o retrabalho real na cor errada —
-- violeta (prioridade 2, "trate isso") em vez de rose (prioridade 1,
-- "solicite isso").
--
-- 'cancelado' entra junto porque hoje ele é pior ainda: não casa nenhum ramo
-- específico e cai no genérico de "tem solicitação e passou de 10 min", ou seja
-- aparece como RETORNO_NÃO_CONFIRMADO — "enviamos e a ASSIM não respondeu",
-- quando nada foi enviado.
--
-- O ESTADO NOVO: 'SOLICITACAO_CANCELADA', prioridade 1 (a mesma de
-- NAO_SOLICITADA, porque a ação exigida é a mesma) e contado no card
-- "Não Solicitadas". Não é um card novo na tela: é um rótulo próprio dentro
-- daquele recorte, para a pessoa saber que ali houve tentativa e ela quebrou —
-- e a observação diz onde quebrou.
--
-- O QUE NÃO MUDA: o relatório da ASSIM continua sendo a palavra final. Se a
-- linha em 'erro' tiver, no relatório (`autorizacoes_assim`), um status que não
-- é 'Liberado', os ramos de `mt` vêm antes e a situação segue GLOSA — como
-- deve, porque aí a recusa foi de fato registrada pela operadora. Reclassificar
-- 'erro' não perde recusa nenhuma: só deixa de inventar as que não existem.
--
-- Corpo idêntico a 20260820150000_glosa_codigos_descricao_completa.sql, exceto
-- os três CASEs (situacao, prioridade, observacao).
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $function$
  ...
  SELECT
    ...
    CASE
      ...
      WHEN fo.status = 'glosa'                            THEN 2
      -- Mesma prioridade de NAO_SOLICITADA: a lacuna é a mesma.
      WHEN fo.status IN ('erro', 'cancelado')             THEN 1
      WHEN fo.paciente_id IS NOT NULL
        AND fo.ultimo_updated_at IS NOT NULL
