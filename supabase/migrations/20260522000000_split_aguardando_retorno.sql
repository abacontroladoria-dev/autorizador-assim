-- Divide AGUARDANDO_RETORNO em dois status operacionais.
-- Base do intervalo: updated_at de fila_autorizacoes (timestamp armazenado em UTC).
-- A conversão AT TIME ZONE 'UTC' garante a comparação correta com NOW() (timestamptz)
-- independente do timezone configurado na sessão do banco.
--
--   SINCRONIZANDO          → NOW() - updated_at <=  10 min
--   RETORNO_NAO_CONFIRMADO → NOW() - updated_at >   10 min

-- ===========================================================================
-- VIEW: vw_auditoria_autorizacoes_assim
-- ===========================================================================
...
WITH fila_operacional AS (
  SELECT
    ...
    MAX(f.updated_at) AS ultimo_updated_at
...
-- ===========================================================================
-- FUNCTION: get_kpis_auditoria_assim
-- ===========================================================================
...
AS $$
  WITH auditoria AS (
    ...
    WHERE data_atendimento = p_data
      AND terapias NOT ILIKE '%Equoterapia%'
      AND terapias NOT ILIKE '%Fisioterapia Aquática%'
  ),
  faltas_dia AS (
  ...
  SELECT
    ...
    count(*) FILTER (WHERE situacao = 'LIBERADA')                 AS liberadas,
    (SELECT total_faltas FROM faltas_dia)                          AS faltas,
    count(*) FILTER (WHERE situacao = 'NAO_SOLICITADA')           AS nao_solicitadas,
    count(*) FILTER (WHERE situacao = 'SINCRONIZANDO')            AS sincronizando,
    count(*) FILTER (WHERE situacao = 'RETORNO_NAO_CONFIRMADO')   AS retorno_nao_confirmado,
    count(*) FILTER (WHERE situacao = 'CANCELADA')                AS canceladas,
    count(*) FILTER (WHERE situacao = 'GLOSA')                    AS glosas
  FROM auditoria;
$$;
