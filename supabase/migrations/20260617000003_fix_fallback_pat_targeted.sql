-- Fix fallback_pat: filtrar LATERAL apenas para pacientes com dados ausentes
--
-- Problema: migration 20260617000002 introduziu LATERAL correto, mas ainda
-- itera sobre TODOS os 145 pacientes do dia. Cada lookup lê 226 linhas em
-- agenda_tita com heap fetch → 69ms × 145 = 10.033ms.
--
-- Causa raiz: o outer subquery usava agenda_tita filtrado por data (todos os
-- pacientes do dia), mesmo que a maioria já tenha cpf/carteirinha hoje.
--
-- Fix: outer subquery passa para agenda_tita_autorizacao (já filtrado por data
-- via índice) e só inclui pacientes onde cpf OU numero_carteirinha está nulo.
-- Se apenas 10-15 pacientes precisam de fallback, são 10-15 lookups em vez de 145.
...
-- ── fallback_pat ───────────────────────────────────────────────────────────────
-- Somente pacientes com cpf OU numero_carteirinha nulos em agenda_tita_autorizacao
-- para p_data. A maioria dos pacientes já tem esses dados (triggers de enriquecimento
-- preenchem na inserção). O LATERAL roda apenas para quem realmente precisa.
fallback_pat AS (
  SELECT
    p.paciente_id,
    ag.cpf,
