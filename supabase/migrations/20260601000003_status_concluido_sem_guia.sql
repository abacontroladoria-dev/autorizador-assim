-- Adiciona suporte ao novo status 'concluido_sem_guia' em vw_central_autorizacoes.
--
-- Contexto: o worker RPA marca 'concluido_sem_guia' quando o ASSIM exibe
-- "BENEFICIO PROCESSADO" mas o regex não consegue extrair o numero_autorizacao
-- da página. Antes, esses casos ficavam como 'concluido' sem número, exibindo
-- o badge verde "Autorizado" indevidamente no Controle de Pacientes.
--
-- Mudanças:
--   status_final: novo WHEN para mapear 'concluido_sem_guia' explicitamente
--   mostrar_na_tela: oculta sessões com 'concluido_sem_guia' da tela Solicitar
--     (o operador não precisa re-processar; o badge "Aguardando guia" aparece
--      apenas no Controle de Pacientes via StatusBadge.tsx)
