-- =============================================================================
-- Reclassificação manual — ETAPA 2: a Conferência consome o override
-- =============================================================================
-- Base: 20260825010000_origem_da_guia_nas_leituras.sql (a definição vigente de
-- get_auditoria_assim_periodo). Corpo idêntico, exceto:
--
--   1. um LATERAL novo (`ovr`), a reclassificação ativa daquele bloco;
--   2. `situacao` ganha UM ramo no topo;
--   3. `prioridade` e `observacao` ganham o ramo correspondente;
--   4. `possui_autorizacao` NÃO muda — ver a nota onde ela mora.
--
-- O match posicional, os LATERAL de glosa e o filtro final não são tocados.
--
-- CARONA DELIBERADA: A FORMA DE VALIDAÇÃO DA GUIA TIRADA NO PORTAL
-- Esta migration corrige, no mesmo CREATE OR REPLACE, um bug independente da
-- reclassificação. Vai junto porque as duas mudanças reescrevem a MESMA função:
-- separá-las em dois arquivos faria a segunda desfazer a primeira, e esta ainda
-- não foi aplicada em lugar nenhum, então incorporar não reescreve história.
--
-- O BUG: sessão cuja autorização foi tirada direto na ASSIM (sem passar pelo
-- Pulsar) nunca mostrava o check de Conferência de Filipetas, mesmo quando houve
-- erro de reconhecimento facial — que deixa papel na recepção, exatamente como o
-- token. Caso real: guia 15032, 03/08/2026, biofacial '1-ERRO NO RECONHECIMENTO FA'.
--
-- A CAUSA, em duas camadas: `forma_autorizacao` só é escrito por UPDATE sobre uma
-- linha existente da fila (20260821080000:262-313) e essa sessão nunca teve linha;
-- e a CTE `autorizacoes` aqui nunca carregava `aa.biofacial`, então o dado existia
-- em autorizacoes_assim e não chegava à tela. Resultado: TabelaAuditoria.tsx:134-135
-- via teve_token=false e forma=NULL, e não desenhava o botão.
--
-- Compare com vw_central_pacientes, que JÁ resolvia isto para o mesmo caso com
-- `'automatico'::text AS forma_autorizacao` (20260825010000:783). A auditoria não
-- tinha equivalente — só herdava o NULL.
--
-- Três pontos abaixo, todos comentados no lugar: `aa.biofacial` na CTE, o repasse
-- por `mt`, e o COALESCE na saída. Mais o GRANT logo aqui embaixo, sem o qual o
-- COALESCE quebraria a aba para todo usuário logado.
--
-- ONDE O RAMO NOVO ENTRA, E POR QUE NO TOPO
-- Acima do vínculo. As duas camadas nunca coexistem numa mesma sessão —
-- reclassificar_situacao() recusa bloco coberto por vínculo, e é lá que essa
-- ordem é decidida —, mas o CASE não pode depender disso: uma linha antiga,
-- gravada antes daquela guarda, ou um vínculo criado por um caminho futuro
-- deixaria a sessão com as duas. Com o override no topo o desempate é explícito
-- e é sempre o mesmo, em vez de depender de qual LATERAL casou primeiro.
--
-- O QUE ESTA MIGRATION MUDA NO SISTEMA, DE VERDADE
-- Tudo que lê `situacao` desta RPC passa a enxergar a reclassificação:
--
--   * os KPIs da Conferência — uma glosa reclassificada como FALTA sai de
--     `glosas` e entra em `faltas`, e sai de `total` (kpisAuditoria.ts:77-81
--     desconta falta do total inteiro);
--   * o resumo diário do cron, que grava a `situacao` crua — e portanto a visão
--     gerencial do mês passa a contar igual à tela do dia, que é a disciplina
--     que 20260824050000 existe para manter;
--   * `fn_alertas_avaliar_assim` — mas SÓ depois de 20260827000002. Sem ela o
--     alerta troca de motivo em vez de fechar. Aplicar as duas juntas.
--
-- VERIFICAÇÃO (o mesmo roteiro de 20260821030000, adaptado)
--   1. contar por situação num dia com glosa: SELECT situacao, count(*)
--      FROM get_auditoria_assim('2026-08-26') GROUP BY 1;
--   2. reclassificar UM bloco em GLOSA para FALTA via reclassificar_situacao();
--   3. repetir (1): exatamente UM bloco muda, de GLOSA para FALTA;
--   4. desfazer_reclassificacao() e repetir (1): volta ao estado inicial.
--
-- VERIFICAÇÃO DA CARONA (o caso real que motivou)
--   5. SELECT guia, teve_token, forma_autorizacao
--        FROM get_auditoria_assim('2026-08-03') WHERE guia = '15032';
--      Esperado: forma_autorizacao = 'Erro no Reconhecimento Facial'
--      (antes desta migration: NULL). Na tela, a linha passa a ter o botão de
--      conferência âmbar, mesmo com teve_token = false.
--
--   6. A precedência do modal NÃO regrediu — o COALESCE não pode ter invertido:
--        SELECT count(*) FROM get_auditoria_assim_periodo('2026-08-01','2026-08-26') a
--        JOIN fila_autorizacoes f ON f.numero_autorizacao = a.guia
--        WHERE f.forma_autorizacao IS NOT NULL
--          AND a.forma_autorizacao IS DISTINCT FROM f.forma_autorizacao;
--      Esperado: 0. Qualquer linha aqui significa que o relatório sobrescreveu a
--      resposta da atendente, que é exatamente o que 20260821080000 proibiu.
--
--   7. O grant pegou (senão a aba inteira cai com 42501 no navegador):
--        SET ROLE authenticated;
--        SELECT public.forma_validacao_do_biofacial('1-ERRO NO RECONHECIMENTO FA', false);
--        RESET ROLE;
--      Esperado: 'Erro no Reconhecimento Facial', sem erro de permissão.
-- =============================================================================

-- Sem DROP: o RETURNS TABLE não muda uma vírgula, então CREATE OR REPLACE basta.
-- (20260825010000 precisou de DROP porque acrescentava `guia_origem`.)

-- ---------------------------------------------------------------------------
-- O grant que esta migration PRECISA fazer antes de mais nada
-- ---------------------------------------------------------------------------
-- 20260821080000:115-116 revogou EXECUTE de PUBLIC e concedeu só a service_role.
-- Fazia sentido: até aqui os únicos consumidores eram sync_assim_results() (cron,
-- service_role) e os snippets de backfill (SQL Editor, service_role).
--
-- get_auditoria_assim_periodo é outra coisa: LANGUAGE sql sem SECURITY DEFINER,
-- com GRANT a anon/authenticated, chamada pelo navegador em
-- frontend/services/auditoria-assim.service.ts:17. Ela executa com os privilégios
-- de QUEM CHAMA. Sem este grant, o COALESCE novo lá embaixo derrubaria a aba
-- inteira com 42501 para todo usuário logado — não um NULL silencioso, um erro.
--
-- O grant é seguro de conceder: forma_validacao_do_biofacial é IMMUTABLE e
-- puramente aritmética sobre um texto (um CASE sobre split_part). Não lê tabela,
-- não toca RLS, não devolve nada que o mesmo usuário já não veja na tela — o
-- rótulo que ela produz é exatamente o que a coluna "Forma" exibe.
GRANT EXECUTE ON FUNCTION public.forma_validacao_do_biofacial(text, boolean)
...
comment on function public.get_auditoria_assim_periodo(date, date) is
  'Conferência ASSIM por período. Considera public.autorizacoes_vinculos (guia vinculada sai do pool posicional e a sessão coberta vira GLOSA_RESOLVIDA ou LIBERADA) e public.auditoria_situacao_overrides (reclassificação manual, que sobrepõe a situação derivada e vem no topo do CASE). Devolve guia_origem: de onde veio a guia exibida (robo = capturada pelo Pulsar no recibo; relatorio = veio do extrato da ASSIM, logo tirada direto no site; reconciliacao = reparo a mao; NULL = nao registrado).';

GRANT EXECUTE ON FUNCTION public.get_auditoria_assim_periodo(date, date) TO anon, authenticated;

-- O wrapper de um dia não é tocado: o RETURNS TABLE não mudou, então o
-- `SELECT *` dele continua válido e ele já enxerga a nova definição.
