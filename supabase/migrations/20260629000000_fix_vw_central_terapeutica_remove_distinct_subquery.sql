-- O timeout ocorre porque a view materializa uma subquery DISTINCT ON que faz
-- full scan em agenda_tita inteira para obter profissional_nome do substituto.
-- O campo profissional_substituto_nome já é gravado diretamente em
-- controle_terapeutico pelo serviço — a subquery é desnecessária.
--
-- Também adicionamos índice composto (data_atendimento, ativo) para garantir
-- que o planner use index-only scan ao filtrar pela data do dia.
