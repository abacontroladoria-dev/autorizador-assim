-- =============================================================================
-- Reconciliação ASSIM — ETAPA 4: a pendência fecha
-- =============================================================================
-- Base: 20260813130200_alerta_glosa_no_aceite.sql (versão vigente de
-- fn_alertas_avaliar_assim). Corpo idêntico, exceto UMA linha do CASE de classe.
--
-- POR QUE ESTA MIGRATION É OBRIGATÓRIA JUNTO DA 20260821030000
-- Sem ela, uma sessão reconciliada sai de GLOSA e cai no `else` do CASE, virando
-- 'pendente_sem_desfecho'. O alerta não fecha: ele TROCA de motivo. A tela diz
-- "glosa resolvida" e o sino continua apontando o mesmo atendimento, agora com
-- o rótulo errado. Aplicar as duas na mesma janela.
--
-- É AQUI que a sessão deixa de ser pendência de faturamento. Não há módulo de
-- faturamento no sistema: o que existe é `public.alertas`, e é o laço de
-- reconciliação desta função (o bloco que compara `av.classe <> g.classe_alvo`)
-- que encerra o alerta `assim_glosa` sozinho, com resolucao='automatico', no
-- ciclo seguinte do cron alertas-assim-avaliar.
--
-- VERIFICAÇÃO: contar alertas por classe antes e depois. Nenhum
-- `assim_sem_desfecho` novo deve nascer, e os `assim_glosa` das sessões
-- vinculadas devem fechar com resolucao='automatico'.
-- =============================================================================
