-- Estende vw_central_pacientes + listar_central_pacientes com dados de SUBSTITUIÇÃO
-- e de quem REALIZOU o atendimento, para o redesign da Central de Operações Clínicas.
--
-- Contexto: a tela precisa responder "quem realizou o atendimento?" e "houve
-- substituição?". Até aqui a RPC só trazia o profissional AGENDADO (profissional_nome)
-- e usuario_nome (que é o nome da MÁQUINA/robô que processou a autorização, não o
-- terapeuta humano). A informação de substituição vive em controle_terapeutico,
-- unida por tita_agendamento_id — mesmo padrão já provado em vw_central_terapeutica
-- (COALESCE(ct.profissional_substituto_nome, agendado)).
--
-- Novas colunas (apêndice no fim do contrato, retrocompatível):
--   profissional_substituto_nome  text         -- nome do substituto, se houve
--   profissional_realizou_nome    text         -- substituto ?? agendado (quem realizou)
--   is_substituicao               boolean      -- houve troca de profissional?
--   controle_status               text         -- status no controle terapêutico
--   confirmado_em                 timestamptz  -- quando o atendimento foi confirmado
--   criado_por                    text         -- atendente que solicitou a autorização (NULL p/ ASSIM automático)
--
-- A função RETURNS SETOF vw_central_pacientes depende do rowtype da view; por isso
-- a função é dropada ANTES de recriar a view, e recriada DEPOIS com o novo formato.

-- Garante lookup eficiente do join lateral (1 linha por agendamento).
CREATE INDEX IF NOT EXISTS idx_controle_terapeutico_tita_agendamento
  ON public.controle_terapeutico (tita_agendamento_id);

DROP FUNCTION IF EXISTS public.listar_central_pacientes(date);
DROP VIEW IF EXISTS public.vw_central_pacientes;

-- ============================================================================
-- VIEW (contrato de tipo) — espelha 20260527210000 + 5 colunas de substituição
-- ============================================================================
...
-- ============================================================================
-- RPC parametrizada — espelha 20260529000007 + 5 colunas, filtrando por p_data
-- ============================================================================
