-- Cria função parametrizada listar_central_pacientes(p_data date).
-- Substitui a query direta na view vw_central_pacientes, que fazia full scan
-- de agenda_tita e autorizacoes_assim sem filtro de data nos CTCs internos
-- (os predicados .eq('data_atendimento') do PostgREST não eram pushados para
-- dentro do subquery com CTCs, causando timeout 57014 e pool esgotado PGRST003).
--
-- A view é mantida para compatibilidade de tipo (RETURNS SETOF vw_central_pacientes).
-- O service passa a chamar supabase.rpc('listar_central_pacientes', { p_data }).
