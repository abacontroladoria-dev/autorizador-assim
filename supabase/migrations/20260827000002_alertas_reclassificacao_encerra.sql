-- =============================================================================
-- Reclassificação manual — ETAPA 3: a pendência de faturamento fecha
-- =============================================================================
-- Base: 20260821050000_alertas_glosa_resolvida_encerra.sql (versão vigente de
-- fn_alertas_avaliar_assim). Corpo idêntico, exceto UM ramo do CASE de classe.
--
-- POR QUE ESTA MIGRATION É OBRIGATÓRIA JUNTO DA 20260827000001
-- Sem ela, uma sessão reclassificada de GLOSA para FALTA sai de 'GLOSA' na RPC
-- mas continua batendo no ramo (4), `fd.tem_glosa` — porque a linha da fila
-- segue em status='glosa', e é assim que tem de ser: o histórico não se apaga.
-- Resultado: o alerta NÃO fecha. Ele fica aberto como pendente_glosa enquanto a
-- tela diz "Falta". É exatamente a armadilha que 20260821050000 documenta para
-- GLOSA_RESOLVIDA, pela mesma razão e com a mesma correção. Aplicar as duas na
-- mesma janela.
--
-- É AQUI QUE "NÃO SERÁ MAIS CONTABILIZADA PELO FATURAMENTO" ACONTECE
-- Não há módulo de faturamento neste sistema: o que existe é `public.alertas`, e
-- é o laço de reconciliação desta função (o bloco que compara
-- `av.classe <> g.classe_alvo`) que encerra o alerta sozinho, com
-- resolucao='automatico', no ciclo seguinte do cron alertas-assim-avaliar. A
-- reclassificação só é real para o faturamento porque passa por aqui.
--
-- O RAMO NOVO NÃO É `s.situacao in ('FALTA', ...)` E ISSO IMPORTA
-- As situações de FALTA nunca chegam a esta função pela via normal: a CTE
-- `blocos_auditoria` da RPC EXCLUI sessão marcada como falta na fila
-- (`agenda_sem_falta`), então uma falta comum simplesmente não existe em `src` —
-- ela sai da lista por ausência, e o laço de reconciliação já a encerra pelo
-- ramo `av.bloco_id is null` ("saiu da agenda"). A sessão RECLASSIFICADA é o
-- caso novo: ela continua em `src`, com a `situacao` sobreposta. Por isso o
-- teste é sobre o conjunto inteiro de destinos permitidos, e por isso ele vem
-- antes de (4).
--
-- VERIFICAÇÃO: contar alertas por classe antes e depois. Nenhum
-- `assim_sem_desfecho` novo deve nascer, e o `assim_glosa` da sessão
-- reclassificada deve fechar com resolucao='automatico'.
-- =============================================================================
  ...
  avaliado as (
    select
      ...
      case
        ...
        -- GLOSA_RESOLVIDA entra AQUI, no ramo (2), e a posição é o ponto todo:
        -- o ramo (4) `fd.tem_glosa` continua verdadeiro para esta sessão (a linha
        -- da fila segue em status='glosa', e é assim que tem de ser — o histórico
        -- não se apaga). Colocar GLOSA_RESOLVIDA depois de (4) não encerraria
        -- nada: a pendência ficaria aberta com a tela dizendo "resolvida".
        -- Vem junto de LIBERADA pela mesma lógica que (4) já assume: evidência
        -- mais nova e mais forte desfaz a classificação anterior.
        when s.situacao in ('LIBERADA', 'CANCELADA', 'GLOSA_RESOLVIDA',
                            -- As duas faltas, vindas de reclassificar_situacao().
                            -- Encerram pelo mesmo motivo que CANCELADA já
                            -- encerrava, uma linha acima: a sessão não
                            -- aconteceu, então não há o que faturar nem o que
                            -- contestar. CANCELADA, o terceiro destino, já
                            -- estava na lista.
                            --
                            -- NAO_SOLICITADA, o quarto, fica FORA de propósito:
                            -- ela é pendência de verdade — a sessão aconteceu e
                            -- segue por solicitar. Cai no else e vira
                            -- pendente_sem_desfecho, que é o alerta certo para
                            -- ela. Reclassificar para NAO_SOLICITADA TROCA o
                            -- alerta de glosa por um de "sem desfecho"; não o
                            -- apaga.
                            'FALTA', 'FALTA_TERAPEUTA')
                                                     then 'concluido'
