-- Detect R5: Substituição via dados TITA (profissional_tratativa != profissional_agendado)
-- Adiciona detecção de substituições que vêm do TITA (evolução por profissional diferente)
-- além das que vêm do controle terapêutico
...
BEGIN
  RETURN QUERY
  -- Fonte 1: substituição registrada no controle terapêutico
  ...
  -- Fonte 2: profissional que evoluiu é diferente do agendado (dados TITA)
...
-- Atualizar get_cco_atendimentos para exibir profissional_tratativa quando não há session_substitutions
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ss.profissional_substituto_nome, a.profissional_tratativa) AS profissional_substituto,
