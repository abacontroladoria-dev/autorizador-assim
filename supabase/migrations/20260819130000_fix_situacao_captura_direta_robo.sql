-- ─────────────────────────────────────────────────────────────────────────────
-- Fix: get_auditoria_assim classificava LIBERADA/GLOSA/CANCELADA só através do
-- match contra autorizacoes_assim (CTE match_temporal/mt). Isso ficou
-- desatualizado desde 20260813100200_robo_rpcs.sql / 20260813130000_robo_conclui_glosa.sql
-- (pedido do usuário, semana de 2026-08-11): o robô agora grava status,
-- numero_autorizacao, horario_autorizacao e status_assim DIRETO em
-- fila_autorizacoes no momento em que lê a confirmação da ASSIM
-- (robo_concluir_tarefa), sem precisar esperar autorizacoes_assim ser
-- sincronizada depois. central-pacientes (vw_central_pacientes) já lê esse
-- dado direto de fila_autorizacoes — auditoria-assim não lia.
--
-- Sintoma relatado: paciente com guia e horário de autorização já visíveis em
-- central-pacientes (fila_autorizacoes.status='concluido' com
-- numero_autorizacao preenchido) aparecia preso em "Retorno Não Confirmado"
-- na auditoria, porque não existe (ainda) uma linha correspondente em
-- autorizacoes_assim.
--
-- 'cancelado' fica de fora de propósito: robo_concluir_tarefa nunca escreve
-- esse status (comentário na própria função: "'cancelado' e 'falta' seguem
-- fora do alcance do robô: são decisões humanas, tomadas na tela") — cancelamento
-- continua dependendo do fluxo antigo via autorizacoes_assim/sync_assim_results.
--
-- Fix: fila_operacional passa a expor status/status_assim/numero_autorizacao/
-- error_message de fila_autorizacoes, e as CASEs de situação/prioridade/
-- observação/guia ganham um branch fo.status='concluido' (e erro/glosa) ANTES
-- do fallback de staleness — só entra em SINCRONIZANDO/RETORNO_NAO_CONFIRMADO
-- quem realmente ainda não tem confirmação nenhuma, direta ou via sync.
--
-- Mesma assinatura e mesmo RETURNS TABLE de antes — CREATE OR REPLACE sem DROP.
-- ─────────────────────────────────────────────────────────────────────────────
