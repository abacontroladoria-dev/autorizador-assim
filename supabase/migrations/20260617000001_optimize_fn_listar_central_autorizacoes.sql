-- Otimização estrutural: elimina double-scan de agenda_tita_autorizacao
--
-- Problema: base e ma_blocos faziam scans independentes da mesma view,
-- duplicando o custo de leitura e do JOIN com agenda_orbita (via fallback_pat).
-- O índice idx_agenda_orbita_paciente_nome mudou o plano do otimizador para
-- uma estratégia mais cara, piorando de 4.1s para 6.5s.
--
-- Solução: CTE raw_slots materializa o scan uma vez (referenciada 2x → PG
-- materializa automaticamente); base e ma_blocos derivam desse resultado
-- já em memória. Elimina uma leitura completa de agenda_tita_autorizacao
-- e uma passagem por agenda_orbita.
...
-- ── raw_slots ──────────────────────────────────────────────────────────────────
-- Scan único de agenda_tita_autorizacao + JOIN fallback_pat.
-- Referenciada por base E ma_blocos → PostgreSQL materializa automaticamente
-- (resultado em memória, reutilizado sem novo acesso à tabela).
raw_slots AS (
  SELECT
    ag.paciente_id,
    ag.paciente_nome,
...
-- ── base ───────────────────────────────────────────────────────────────────────
-- Deriva de raw_slots (in-memory) — sem novo scan de agenda_tita_autorizacao.
base AS (
  SELECT
    rs.paciente_id,
    rs.paciente_nome,
...
-- ── ma_blocos ──────────────────────────────────────────────────────────────────
-- Deriva de raw_slots (in-memory) — sem novo scan de agenda_tita_autorizacao.
-- Usa rs.matricula / rs.dep raw (mesmo que ag.matricula / ag.dep).
ma_blocos AS (
  SELECT
    rs.paciente_id,
    rs.data_atendimento,
