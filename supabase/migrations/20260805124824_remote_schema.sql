drop trigger if exists "controle-terapeutico-slack" on "public"."controle_terapeutico";

drop policy "cronograma_salas_write" on "public"."cronograma_salas";

drop policy "cronograma_salas_alocacoes_write" on "public"."cronograma_salas_alocacoes";
...
-- Replay fix: listar_central_pacientes(date) retorna SETOF vw_central_pacientes,
-- então depende do TIPO da view — o `supabase db diff` não enxerga essa
-- dependência e gerava só o drop da view, que falhava com 2BP01. Dropar a função
-- antes resolve; ela é recriada no fim deste arquivo (definição atual de prod).
-- NÃO trocar por "drop ... cascade": a função sumiria sem ser recriada.
drop function if exists "public"."listar_central_pacientes"("p_data" "date");
...
drop view if exists "public"."agenda_tita_autorizacao";
  ...
  create table "public"."acomp_auditoria" (
...
alter table "public"."acomp_auditoria" enable row level security;

CREATE UNIQUE INDEX acomp_auditoria_pkey ON public.acomp_auditoria USING btree (id);

CREATE INDEX idx_acomp_auditoria_criado_em ON public.acomp_auditoria USING btree (criado_em DESC);

CREATE INDEX idx_acomp_auditoria_evento ON public.acomp_auditoria USING btree (evento);

CREATE INDEX idx_acomp_auditoria_lote ON public.acomp_auditoria USING btree (lote_id);

CREATE INDEX idx_acomp_auditoria_paciente ON public.acomp_auditoria USING btree (paciente);

CREATE INDEX idx_acomp_auditoria_usuario ON public.acomp_auditoria USING btree (usuario_id);

alter table "public"."acomp_auditoria" add constraint "acomp_auditoria_pkey" PRIMARY KEY using index "acomp_auditoria_pkey";

alter table "public"."acomp_auditoria" add constraint "acomp_auditoria_evento_check" CHECK ((evento = ANY (ARRAY['aceite'::text, 'escrita_tita'::text]))) not valid;

alter table "public"."acomp_auditoria" validate constraint "acomp_auditoria_evento_check";

alter table "public"."acomp_auditoria" add constraint "acomp_auditoria_usuario_id_fkey" FOREIGN KEY (usuario_id) REFERENCES auth.users(id) not valid;

alter table "public"."acomp_auditoria" validate constraint "acomp_auditoria_usuario_id_fkey";

set check_function_bodies = off;

create or replace view "public"."vw_acomp_auditoria" as  SELECT id,
    evento,
    to_char((criado_em AT TIME ZONE 'America/Sao_Paulo'::text), 'DD/MM/YYYY'::text) AS data,
    to_char((criado_em AT TIME ZONE 'America/Sao_Paulo'::text), 'HH24:MI:SS'::text) AS hora_registro,
...
grant delete on table "public"."acomp_auditoria" to "anon";

grant insert on table "public"."acomp_auditoria" to "anon";

grant references on table "public"."acomp_auditoria" to "anon";

grant select on table "public"."acomp_auditoria" to "anon";

grant trigger on table "public"."acomp_auditoria" to "anon";

grant truncate on table "public"."acomp_auditoria" to "anon";

grant update on table "public"."acomp_auditoria" to "anon";

grant delete on table "public"."acomp_auditoria" to "authenticated";

grant insert on table "public"."acomp_auditoria" to "authenticated";

grant references on table "public"."acomp_auditoria" to "authenticated";

grant select on table "public"."acomp_auditoria" to "authenticated";

grant trigger on table "public"."acomp_auditoria" to "authenticated";

grant truncate on table "public"."acomp_auditoria" to "authenticated";

grant update on table "public"."acomp_auditoria" to "authenticated";

grant delete on table "public"."acomp_auditoria" to "service_role";

grant insert on table "public"."acomp_auditoria" to "service_role";

grant references on table "public"."acomp_auditoria" to "service_role";

grant select on table "public"."acomp_auditoria" to "service_role";

grant trigger on table "public"."acomp_auditoria" to "service_role";

grant truncate on table "public"."acomp_auditoria" to "service_role";

grant update on table "public"."acomp_auditoria" to "service_role";


  create policy "acomp_auditoria insert"
  on "public"."acomp_auditoria"
  as permissive
  for insert
  to authenticated
with check ((usuario_id = auth.uid()));
  ...
  create policy "acomp_auditoria select"
  on "public"."acomp_auditoria"
  as permissive
  for select
  to authenticated
using (true);
  ...
  create policy "cronograma_salas_write"
  on "public"."cronograma_salas"
  as permissive
  for all
  to authenticated
using (public.remuneracao_has_role(ARRAY['admin'::text, 'diretoria'::text, 'cronograma'::text, 'terapeutico'::text]))
with check (public.remuneracao_has_role(ARRAY['admin'::text, 'diretoria'::text, 'cronograma'::text, 'terapeutico'::text]));



  create policy "cronograma_salas_alocacoes_write"
  on "public"."cronograma_salas_alocacoes"
  as permissive
  for all
  to authenticated
using (public.remuneracao_has_role(ARRAY['admin'::text, 'diretoria'::text, 'cronograma'::text, 'terapeutico'::text]))
...
-- Replay fix (continuação): recria listar_central_pacientes com a definição
-- atual de produção, depois que vw_central_pacientes já foi recriada acima.
