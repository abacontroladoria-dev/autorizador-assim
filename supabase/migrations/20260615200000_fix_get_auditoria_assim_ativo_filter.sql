-- Fix: get_auditoria_assim incluía sessões ativo=false de agenda_tita, deslocando
-- a numeração ordem_sessao e quebrando o match_temporal com autorizacoes_assim.
-- O bot que preenche autorizacoes_assim usa agenda_tita_autorizacao (ativo=true),
-- então a função de auditoria deve usar o mesmo universo.
-- Regra: para pacientes ASSIM, uma sessão = uma autorização.
