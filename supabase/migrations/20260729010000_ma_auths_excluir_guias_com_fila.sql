-- ITEM 5 — /solicitar: guia já reivindicada por uma fila não casa mais posicionalmente.
--
-- O ASSIM carimba data_execucao = HOJE em toda guia (não tem campo de data), então
-- uma autorização RETROATIVA (feita hoje) casava, em ma_auths, com o atendimento de
-- HOJE da mesma especialidade → o de hoje sumia (autorizado_externo) indevidamente.
--
-- Agora o robô (rpa.js) grava o nº da guia em fila_autorizacoes.numero_autorizacao
-- (âncora à sessão real, cuja data está na fila). Este patch faz ma_auths EXCLUIR do
-- pareamento posicional as guias já vinculadas a uma fila (de qualquer data) — mesmo
-- padrão que listar_central_pacientes.guias_sem_fila já usa. Assim a guia retroativa
-- deixa de casar com o atendimento de hoje; o pareamento posicional passa a servir
-- apenas às guias "sem fila" (autorização feita direto no portal ASSIM).
--
-- Recriação FIEL da definição viva (pg_get_functiondef) + a única adição: o NOT EXISTS
-- no WHERE de ma_auths.
