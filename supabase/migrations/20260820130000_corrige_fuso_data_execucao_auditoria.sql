-- ─────────────────────────────────────────────────────────────────────────────
-- Corrige o fuso de data_execucao (e de autorizacao_updated_at) na auditoria.
--
-- SINTOMA: no modal de detalhamento, "Autorizado em" e "Executado em" mostravam
-- horarios 3h diferentes para o MESMO evento. Medido em 124 linhas de
-- 2026-08-20: o delta era exatamente -180,0 min em todas.
--
-- CAUSA: `autorizacoes_assim.data_execucao` e `.updated_at` sao
-- `timestamp WITHOUT time zone` guardando hora de Sao Paulo (conferido no
-- catalogo da base viva). A RETURNS TABLE desta funcao declara as duas como
-- `timestamp WITH time zone`, entao o Postgres faz o cast implicito usando o
-- TimeZone da sessao — UTC no Supabase. O valor 08:13 (Sao Paulo) sai
-- carimbado como 08:13+00, o PostgREST serializa "...+00:00" e o navegador
-- converte para 05:13. O dado no banco esta correto; quem mentia era o cast.
--
-- `horario_autorizacao` escapava do problema por ser declarado
-- `timestamp WITHOUT time zone`: sai sem sufixo e o JS o le como local.
--
-- CORRECAO: converter explicitamente no SELECT final. `AT TIME ZONE
-- 'America/Sao_Paulo'` sobre um timestamp naive devolve timestamptz
-- interpretando-o como horario de Sao Paulo — que e o que ele de fato e.
-- Passa a valer o instante verdadeiro, e a exibicao fecha com
-- "Autorizado em".
--
-- POR QUE NAO MUDAR A ASSINATURA PARA `WITHOUT time zone`: trocar tipo de
-- coluna de retorno exige DROP FUNCTION, e get_auditoria_assim(date) e
-- get_tokens_mensal(date) dependem desta. Seriam 3 funcoes derrubadas e
-- ~300 linhas recriadas, perdendo os grants no caminho. A conversao explicita
-- resolve a mesma causa com CREATE OR REPLACE.
--
-- ALCANCE: as outras duas funcoes herdam a correcao sem serem tocadas —
-- get_auditoria_assim faz `SELECT *` daqui e get_tokens_mensal projeta
-- `p.data_execucao` daqui.
--
-- O resto do corpo e copia byte a byte de
-- 20260820110000_encurta_observacao_solicitacao_enviada.sql, que era a
-- definicao vigente. So as duas linhas da projecao mudam.
-- ─────────────────────────────────────────────────────────────────────────────
