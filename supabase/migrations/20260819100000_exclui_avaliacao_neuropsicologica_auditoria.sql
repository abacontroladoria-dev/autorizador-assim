-- ─────────────────────────────────────────────────────────────────────────────
-- Exclui "Avaliação Neuropsicológica" das visualizações de /auditoria-assim
--
-- Mesmo mecanismo já usado para Equoterapia e Fisioterapia Aquática (ver
-- 20260813120000_tuss_da_sessao_fallback_por_terapia.sql, WHERE final): um
-- NOT ILIKE sobre `b.terapias` (string_agg dos terapia_exibicao_nome do bloco).
-- Pedido do usuário em 2026-08-19: essa terapia não deve aparecer nem na
-- tabela nem nos KPIs da aba Auditoria.
--
-- Recriação FIEL da função anterior — única mudança é a linha nova no WHERE
-- final. Todo o resto (CTEs, JOINs, CASE de situação/prioridade) é idêntico.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Mesma exclusão em get_faltas_auditoria_assim (20260805124824_remote_schema.sql,
-- linhas 1022-1055): faltas dessa terapia entram no rawDados do hook por este
-- RPC separado (listarFaltasAuditoria) e alimentavam os KPIs de falta mesmo
-- depois da exclusão acima em get_auditoria_assim.
-- ─────────────────────────────────────────────────────────────────────────────
