-- ─────────────────────────────────────────────────────────────────────────────
-- Encurta a observação do estado SINCRONIZANDO.
--
-- Antes: 'Solicitação enviada. Aguardando sincronização com a ASSIM.'
-- Agora: 'Solicitação enviada.'
--
-- Pedido do usuário (2026-08-20). A segunda frase repetia o que o badge da
-- linha já diz — a situação "Sincronizando" É a espera pela ASSIM — e era a
-- observação mais comprida da tabela, forçando truncamento na coluna e no
-- rodapé do modal de detalhamento.
--
-- Só a literal muda. O resto de get_auditoria_assim_periodo é cópia byte a
-- byte de 20260819130000_fix_situacao_captura_direta_robo.sql, que é a
-- definição vigente (nenhuma migration posterior a redefine — 140000 e 150000
-- e 20260820100000 mexem apenas em get_tokens_mensal).
--
-- get_auditoria_assim(p_data) não precisa ser tocada: é wrapper que delega
-- para esta com p_data_inicio = p_data_fim.
--
-- CREATE OR REPLACE preserva a assinatura; sem DROP, para não derrubar as
