-- ─────────────────────────────────────────────────────────────────────────────
-- Detalhamento do atendimento em /auditoria-assim?tab=auditoria (pedido pós-reunião,
-- 2026-08-19): botão de detalhe passa a existir em TODOS os status (não só GLOSA)
-- e o modal ganha um campo de observações livre + marcação de "conferido" para
-- atendimentos com token (filipeta impressa/assinada).
--
-- Duas tabelas satélite por bloco_id, mesmo padrão de auditoria_glosa_motivos
-- (bloco_id é sintético — concat_ws('_', paciente_id, data, tuss, hora) — não
-- existe em nenhuma tabela real, então não pode ser FK). RLS aberta a
-- authenticated, mesmo padrão de cronograma_paciente_observacoes (20260818170000).
-- ─────────────────────────────────────────────────────────────────────────────

create table if not exists public.auditoria_atendimento_notas (
  id                uuid primary key default gen_random_uuid(),
  bloco_id          text not null unique,
  texto             text not null,
...
alter table public.auditoria_atendimento_notas enable row level security;

drop policy if exists "auditoria_atendimento_notas_all" on public.auditoria_atendimento_notas;
create policy "auditoria_atendimento_notas_all" on public.auditoria_atendimento_notas
  for all to authenticated using (true) with check (true);

create table if not exists public.auditoria_token_conferencias (
  id                  uuid primary key default gen_random_uuid(),
  bloco_id            text not null unique,
...
alter table public.auditoria_token_conferencias enable row level security;

drop policy if exists "auditoria_token_conferencias_all" on public.auditoria_token_conferencias;
create policy "auditoria_token_conferencias_all" on public.auditoria_token_conferencias
  for all to authenticated using (true) with check (true);

-- ─────────────────────────────────────────────────────────────────────────────
-- get_auditoria_assim: expõe criado_por/forma_autorizacao/horario_autorizacao
-- (já existem em fila_autorizacoes, só não eram devolvidos) para a seção
-- "Autorização" do novo modal, nos moldes do SidePanel de central-pacientes
-- (Solicitado por / Forma / Autorizado em). fila_operacional deixa de ser um
-- GROUP BY/max(...) e vira DISTINCT ON pela linha mais recente por chave —
-- mesmo resultado para ultimo_updated_at, só muda a forma de calcular, e agora
-- dá pra carregar junto as 3 colunas novas sem duplicar linha.
--
-- Recriação FIEL da função anterior (20260819100000) — DROP necessário porque
-- o RETURNS TABLE muda (mesmo motivo de 20260615130000_add_token_to_auditoria_assim.sql).
--
-- Correção (mesmo dia): o join de fila_operacional usava empresa/matricula/dep
-- (parseados da carteirinha em blocos_auditoria vs. gravados direto em
-- fila_autorizacoes) — chave frágil, casava errado silenciosamente e devolvia
-- criado_por/forma_autorizacao/horario_autorizacao nulos mesmo quando a linha
-- existia. vw_central_pacientes (20260805170100) já resolve esse mesmo tipo de
-- linha via paciente_id+data+horario, não carteirinha — troquei para o mesmo
-- padrão, mantendo codigo_tuss/horario para não perder a granularidade por sessão.
-- ─────────────────────────────────────────────────────────────────────────────
