-- =============================================================================
-- `8-DISPOSITIVO INDISPONIVEL` exige filipeta, com ou sem token
-- =============================================================================
-- Base: 20260902110000_glosa_sem_papel_sai_da_conferencia.sql (a definição
-- vigente). Corpo idêntico, exceto o que está listado em A CORREÇÃO.
--
-- A REGRA NOVA
-- Sempre que `autorizacoes_assim.biofacial` vier com o prefixo `8-`
-- (8-DISPOSITIVO INDISPONIVEL), a sessão exige conferência de filipeta.
--
-- POR QUE ELA NÃO ERA ATENDIDA
-- O `8-` já entrava na Conferência, mas SÓ pela porta do token. A cadeia é a
-- que 20260821080000:101-105 documenta: sem dispositivo, a ASSIM cai no
-- #checkBday e emite a filipeta — o papel é consequência do `8-`, e o token é
-- consequência do papel. Daí a Semente 1 (`teve_token = true`) pegar a maioria
-- dos casos de graça.
--
-- Mas "a maioria" não é "sempre", e o próprio de-para mede o buraco: dos 106
-- casos de `8-` na janela de 21/08/2026, 97 tinham token — os outros NOVE não.
-- Esses nove emitiram filipeta (o `8-` é a causa do papel, não o token) e
-- ficavam fora da Conferência em silêncio, porque nenhuma das três sementes
-- olha o biofacial de uma guia SEM vínculo:
--
--   Semente 1 (chaves_token)   `teve_token = true`            -> não pega
--   Semente 2 (chaves_facial)  fila, '%reconhecimento facial%' -> não pega
--   Semente 3 (chaves_vinculo) exige vínculo ativo             -> não pega
--
-- É o mesmo silêncio de sempre nesta família: a sessão não aparece errada, ela
-- não aparece.
--
-- A CORREÇÃO — quatro pontos, todos mecânicos
--
-- 1. `auth_mes` passa a projetar `aa.biofacial` (não projetava).
-- 2. SEMENTE 4 NOVA (`chaves_dispositivo`): partições do mês cujo biofacial tem
--    prefixo `8-`, sem olhar token. Entra no UNION de `chaves` como as outras.
-- 3. `autorizacoes` e `match_temporal` passam a carregar `biofacial`, para o
--    WHERE final poder ler o da guia pareada por POSIÇÃO. Sem isso a Semente 4
--    traria o dia para dentro e o WHERE derrubaria a linha logo depois — seed e
--    filtro têm de concordar, senão a partição entra e a sessão não.
-- 4. O WHERE final ganha o ramo do `8-`, lendo vínculo primeiro e posicional
--    como fallback, na MESMA ordem de precedência que 20260827000004 fixou para
--    guia/token/forma (`COALESCE(vin.*, mt.*)`).
--
-- POR QUE O RAMO NOVO NÃO PRECISA DO GATE DE RECUSA
-- Mesma razão dada em 20260902110000:27-35 para o ramo do token: `biofacial` é
-- campo do RELATÓRIO da ASSIM, não do modal da recepção. Ele só existe porque a
-- ASSIM processou a autorização e descreveu como o beneficiário foi validado —
-- é resposta, não intenção. Só o ramo da FILA (`fo.forma_autorizacao`) opina
-- sobre fato que a ASSIM ainda não confirmou, e por isso só ele carrega o gate.
--
-- Ainda assim o `8-` sob recusa não escapa: a guia recusada não entra em
-- `autorizacoes` com `ordem_autorizacao` casando a sessão por posição na
-- prática comum, e onde entrar, o papel do #checkBday de fato saiu — a filipeta
-- documenta a tentativa, que é o que a recepção tem em mãos.
--
-- O PREFIXO, NUNCA O RÓTULO
-- `split_part(btrim(...), '-', 1) = '8'`, idêntico ao de
-- `forma_validacao_do_biofacial`. O rótulo do extrato vem truncado em 25 chars
-- ('8-DISPOSITIVO INDISPONIVEL' tem 26 e chega cortado), o vocabulário não é
-- fechado e o `8` é o único pedaço estável. Ver
-- reference_biofacial_no_extrato_assim.
--
-- Não se usa `forma_validacao_do_biofacial(...) = 'Dispositivo indisponível'`
-- de propósito: aquela função devolve 'Token' quando `8-` vem com token, então
-- casar pelo rótulo perderia exatamente os 97 casos com token — o inverso do
-- que esta migration pede.
--
-- BLAST RADIUS
-- Só ACRESCENTA linhas à Conferência de Filipetas; nenhuma sai. Nada muda para
-- quem já entrava por token, fila ou vínculo. `get_tokens_mensal` é lida pela
-- página de Conferência de Filipetas e não alimenta faturamento.
--
-- CONTRAPROVA (rodar antes e depois, na mesma sessão)
--   select count(*) from public.get_tokens_mensal('2026-08-01');
--
-- Quantas linhas o `8-` sem token acrescenta num mês:
--   select count(distinct concat_ws('_',
--            split_part(aa.matricula,'.',1), split_part(aa.matricula,'.',2),
--            split_part(aa.matricula,'.',3), date(aa.data_execucao), aa.codigo_tuss))
--   from public.autorizacoes_assim aa
--   where date(aa.data_execucao) >= '2026-08-01'
--     and date(aa.data_execucao) <  '2026-09-01'
