-- ─────────────────────────────────────────────────────────────────────────────
-- O motivo da glosa lido no recibo do aceite chega à auditoria por extenso.
--
-- SINTOMA: quando a ASSIM recusa no ato do envio ("BENEFICIO REJEITADO"), o robô
-- lê o motivo colado no TUSS — "(1013) CADASTRO DO BENEFICIARIO COM PROBLEMAS" —
-- e grava em `fila_autorizacoes.status_assim` no formato "1013-CADASTRO DO
-- BENEFICIARIO COM PROBLEMAS" (rpa.js, lerConfirmacao). Mesmo assim a auditoria
-- mostrava "Glosa: Erro não identificado": o ramo de glosa da observação lia
-- `fo.error_message`, que numa recusa carrega perícia de captura ("Guia da
-- rejeição não capturada"), não o motivo. E `codigo_erro`/`descricao_erro`
-- saíam nulos porque só eram projetados de `match_temporal` — ou seja, só
-- existiam depois que o relatório da ASSIM chegasse, horas mais tarde.
--
-- CORREÇÃO: decompor `fo.status_assim` da linha em glosa nos mesmos dois campos
-- que o relatório preenche. O robô já grava no vocabulário do relatório
-- ("1601-REINCIDENCIA NO ATEN"), então o par código/descrição sai de um
-- `split` no primeiro hífen — a mesma decomposição que
-- 20260813130200_alerta_glosa_no_aceite.sql já faz para o snapshot do alerta.
--
-- POR QUE PREENCHER AS COLUNAS EXISTENTES em vez de criar uma nova: `codigo_erro`
-- e `descricao_erro` JÁ significam exatamente isto — o código e o texto por
-- extenso da recusa do convênio. Toda a tela que os consome (a legenda do card,
-- o rodapé vermelho do modal de detalhamento) passa a mostrar o motivo sem
-- mudança nenhuma de contrato. E preencher coluna existente é CREATE OR REPLACE;
-- coluna nova exigiria DROP das três funções dependentes, com os grants junto.
--
-- PRECEDÊNCIA: `mt.*` continua vencendo. O recibo diz "sujeito a análise
-- posterior", então quando o relatório chega é ele quem manda — o COALESCE
-- coloca o recibo como o valor que vale ENQUANTO o relatório não existe.
--
-- DEPENDE DE: supabase/snippets/20260813_glosa_no_aceite.sql (é ele que ensina
-- `robo_concluir_tarefa` a aceitar 'glosa' e a gravar `p_status_assim`). Sem
-- aquele bloco aplicado nenhuma linha desta forma existe, e esta migration é
-- inócua — não quebra nada, apenas não tem o que mostrar.
--
-- JUNTO VAI UMA CORREÇÃO QUE ESTA MUDANÇA TORNOU NECESSÁRIA: o filtro de faltas
-- (em `agenda_sem_falta` e em `fila_operacional`) testava
-- `status_assim LIKE '%FALTA%'`. Isso era seguro enquanto a coluna só guardava
-- rótulos curtos; agora que ela carrega o motivo por extenso, uma recusa do tipo
-- "FALTA DE COBERTURA CONTRATUAL" faria a sessão inteira desaparecer da
-- auditoria — calada — exatamente no caso que mais precisa ser visto. O teste de
-- texto passa a não valer para linha em 'glosa'. `tipo_falta` continua intocado:
-- é coluna dedicada e não sofre do problema.
--
-- O resto do corpo é cópia byte a byte de
-- 20260820130000_corrige_fuso_data_execucao_auditoria.sql, que era a definição
-- vigente. Mudam só a CTE `fila_operacional` (duas colunas novas), os dois
-- filtros de falta e três linhas da projeção final.
-- ─────────────────────────────────────────────────────────────────────────────
