-- Adiciona filtro a.ativo = TRUE para eliminar agendamentos cancelados/substituídos
-- no nível do TITA que geravam sessões duplicadas no modal de cobertura.
-- Padrão idêntico ao usado em agenda_tita_autorizacao (migration 20260527110000).
--
-- Também usa DISTINCT ON no subquery do substituto para evitar fan-out quando
-- o mesmo profissional_id tem variações ortográficas no profissional_nome.
-- Inclui confirmado_por_nome para compatibilidade com a coluna já existente no remote.
