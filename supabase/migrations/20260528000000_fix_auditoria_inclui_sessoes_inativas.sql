-- Recria vw_auditoria_autorizacoes_assim usando agenda_tita diretamente
-- (sem filtro ativo) para incluir sessões substituídas na grade que tiveram
-- autorizações tentadas (glosas, canceladas) em autorizacoes_assim.
--
-- Problema: migration 20260527110000 adicionou ativo=true em agenda_tita_autorizacao,
-- que é a base de vw_blocos_autorizaveis_assim, que é a base desta view. Sessões com
-- ativo=false (substituídas na grade) desapareceram do audit mesmo tendo autorizações.
--
-- Solução: o CTE blocos_auditoria lê agenda_tita (todos os ativo) com o mesmo
-- mapeamento TUSS e mesmas regras de filtro do vw_blocos_autorizaveis_assim.
-- vw_blocos_autorizaveis_assim NÃO é alterada — o filtro ativo=true nela
-- é correto para evitar terapias duplicadas nos cards da home/solicitar.
...
blocos_auditoria AS (
  WITH agenda_tita_tuss AS (
    SELECT
      ...
      CASE
        WHEN at.terapia_exibicao_nome = ANY (ARRAY[
          'Psicologia'::text, 'Psicologia ABA'::text,
          'Arteterapia'::text, 'Arteterapia (Psicologia ABA)'::text,
          'Avaliação Neuropsicológica'::text, 'Habilidades Sociais (Psicologia ABA)'::text
        ]) THEN '22070384'::text
    ...
    WHERE at.convenio_nome ILIKE '%assim%'
      AND at.paciente_nome NOT IN ('Horário Administrativo', 'Notificação Prévia')
  ),
  agenda_filtrada AS (
    SELECT a.*
  ...
  SELECT
    concat_ws('_', paciente_id, data_atendimento, codigo_tuss, hora_inicial) AS bloco_id,
    paciente_id,
    paciente_nome,
    empresa,
    matricula,
    dep,
    concat_ws('.', empresa, matricula, dep) AS carteirinha,
    data_atendimento,
    hora_inicial,
    codigo_tuss,
    convenio_nome,
  ...
  GROUP BY paciente_id, paciente_nome, empresa, matricula, dep,
           data_atendimento, hora_inicial, codigo_tuss, convenio_nome
),
