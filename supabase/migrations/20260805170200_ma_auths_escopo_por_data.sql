...
-- (20260804020000_filtrar_pacientes_por_unidade_recepcao.sql, da linha 26 em diante —
-- o ALTER/UPDATE em `usuarios` daquela migration NÃO é repetido aqui) com uma única
-- mudança: o NOT EXISTS de `ma_auths` deixa de comparar o número da guia isolado.
--
-- O número da guia da ASSIM não é único ao longo do tempo: 4.652 números aparecem em
-- mais de uma linha de fila em produção, cobrindo 12.883 linhas. Comparando só o
-- número, uma guia legítima de hoje era considerada "já vinculada" porque uma linha
-- não relacionada de meses atrás carregava o mesmo valor — e a sessão deixava de ser
-- reconhecida como 'autorizado_externo', voltando a aparecer em /solicitar como se
-- precisasse de ação. Escopando por data, a exclusão volta a significar o que
-- pretendia: "esta guia já pertence a uma fila deste mesmo período".
--
-- Mesma correção aplicada à Parte 2 de listar_central_pacientes em 20260805170100.
