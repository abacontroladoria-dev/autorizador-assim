-- Recriação FIEL de 20260804050000_add_confirmado_por_nome_central_pacientes.sql
-- com duas mudanças, ambas motivadas pelo caso Alvaro Soares Da Silva (05/08/2026):
--
-- (1) status_operacional deixa de mentir. Hoje `fa.status='concluido'` vira
--     'autorizado' mesmo com `numero_autorizacao IS NULL`, então o card fica verde
--     "Autorizado" com o campo Guia vazio e nada sinaliza a lacuna. Passa a devolver
--     'concluido_sem_guia' — que severity.ts:131 já mapeia como *andamento*
--     ("aguardando guia"), e que central-pacientes/page.tsx:181,307 já filtra.
--     O ramo entra ANTES de `status_assim='autorizado'` e de `status='concluido'`.
--
-- (2) O NOT EXISTS de `guias_sem_fila` deixa de comparar o número da guia cru.
--     O número da ASSIM NÃO é único ao longo do tempo: em produção, 4.652 números
--     aparecem em mais de uma linha de fila, cobrindo 12.883 linhas (até 5
--     repetições do mesmo valor). Exemplo real: a guia 3883 pertence a Ícaro
--     (01/06), Kourtney (01/07) e Alvaro (03/08). Como a exclusão casava só o
--     número, uma guia legítima de hoje era descartada porque uma linha não
--     relacionada de junho carregava o mesmo valor — a Parte 2 perdia guias
--     silenciosamente para qualquer paciente. Agora a exclusão é escopada por data.
