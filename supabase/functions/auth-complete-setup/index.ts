...
serve(async (req: Request) => {
  ...
  const { error: updateError } = await supabaseAdmin
    .from("usuarios")
    .update({ username: username.toLowerCase(), ativo: true, primeiro_acesso: false })
    .eq("id", user.id);

  if (updateError) {
    return jsonResponse({ error: "update_failed", message: updateError.message }, 500, corsHeaders);
  }
